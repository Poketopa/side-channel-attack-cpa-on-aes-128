#include <stdio.h>
#include <stdlib.h>

typedef unsigned char Byte;

Byte S[4][256] = {
    {
  },
{
  },
{
  },
{
  }
};

Byte KRK[3][16] = {
    {0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0},
    {0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0},
    {0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e}
};

Byte masking_Sbox[4][256] = { {0x00,}, };

void generateMaskingSbox(Byte S[4][256], Byte m_1, Byte m_2, Byte masking_Sbox[3][256]) {
    for (int i = 0; i < 256; i++) {
        Byte masking_index = (Byte)i ^ m_1;
        Byte making_Sbox_m_1 = S[0][i] ^ m_2;
        Byte making_Sbox_m_2 = S[1][i] ^ m_2;

        masking_Sbox[0][masking_index] = making_Sbox_m_1;
        masking_Sbox[1][masking_index] = making_Sbox_m_2;
        masking_Sbox[2][making_Sbox_m_1] = masking_index;
        masking_Sbox[3][making_Sbox_m_2] = masking_index;
    }
}

void DL(Byte *i, Byte *o) {
    Byte T;
    T = i[3] ^ i[4] ^ i[9] ^ i[14];
    o[0] = i[6] ^ i[8] ^ i[13] ^ T;
    o[5] = i[1] ^ i[10] ^ i[15] ^ T;
    o[11] = i[2] ^ i[7] ^ i[12] ^ T;
    o[14] = i[0] ^ i[5] ^ i[11] ^ T;
    T = i[2] ^ i[5] ^ i[8] ^ i[15];
    o[1] = i[7] ^ i[9] ^ i[12] ^ T;
    o[4] = i[0] ^ i[11] ^ i[14] ^ T;
    o[10] = i[3] ^ i[6] ^ i[13] ^ T;
    o[15] = i[1] ^ i[4] ^ i[10] ^ T;
    T = i[1] ^ i[6] ^ i[11] ^ i[12];
    o[2] = i[4] ^ i[10] ^ i[15] ^ T;
    o[7] = i[3] ^ i[8] ^ i[13] ^ T;
    o[9] = i[0] ^ i[5] ^ i[14] ^ T;
    o[12] = i[2] ^ i[7] ^ i[9] ^ T;
    T = i[0] ^ i[7] ^ i[10] ^ i[13];
    o[3] = i[5] ^ i[11] ^ i[14] ^ T;
    o[6] = i[2] ^ i[9] ^ i[12] ^ T;
    o[8] = i[1] ^ i[4] ^ i[15] ^ T;
    o[13] = i[3] ^ i[6] ^ i[8] ^ T;
}

void RotXOR(Byte *s, int n, Byte *t) {
    int i, q;
    q = n / 8;
    n %= 8;
    for (i = 0; i < 16; i++) {
        t[(q + i) % 16] ^= (s[i] >> n);
        if (n != 0) t[(q + i + 1) % 16] ^= (s[i] << (8 - n));
    }
}

void printBlock(Byte *b) {
    for (int i = 0; i < 16; i++) {
        printf("%02x", b[i]);
        if (i % 4 == 3 && i < 15) printf(" ");
    }
    printf("\n");
}

int EncKeySetup(Byte* w0, Byte* e, int keyBits, Byte m_1, Byte m_2, Byte masking_xor) {
   int  i, R = (keyBits + 256) / 32, q;
   Byte t[16], w1[16], w2[16], w3[16];
   for (i = 0; i < 16; i++) {
      w0[i] ^= masking_xor;
   }

   for (i = 16; i < 32; i++) {
      if (i == 16 || i == 17 || i == 22 || i == 23 || 
          i == 24 || i == 25 || i == 30 || i == 31) {
            w0[i] ^= m_2;
      }
      else {
        w0[i] ^= m_1;
      }
   }


   q = (keyBits - 128) / 64;
   for (i = 0; i < 16; i++) {
      if (i % 4 == 0 || i % 4 == 1)
         t[i] = masking_Sbox[i % 4][KRK[q][i] ^ w0[i] ^ m_2];
      else t[i] = masking_Sbox[i % 4][KRK[q][i] ^ w0[i] ^ m_1];
   }

   DL(t, w1);
   for (i = 0; i < 16; i++) {
     w1[i] ^= w0[i + 16];
   }

   q = (q == 2) ? 0 : (q + 1);
   for (i = 0; i < 16; i++) {
      if (i % 4 == 0 || i % 4 == 1) t[i] = masking_Sbox[(2 + i) % 4][KRK[q][i] ^ w1[i] ^ m_1];
      else t[i] = masking_Sbox[(2 + i) % 4][KRK[q][i] ^ w1[i] ^ m_2];
   }
   DL(t, w2);
   for (i = 0; i < 16; i++) {
      if (i == 0 || i == 1 || i == 6 || i == 7 || i == 8 || i == 9 || i == 14 || i == 15) w2[i] ^= m_2 ^ w0[i];
      else w2[i] ^= m_1 ^ w0[i];
   }


   q = (q == 2) ? 0 : (q + 1);
   for (i = 0; i < 16; i++) {
      if (i % 4 == 0 || i % 4 == 1) t[i] = masking_Sbox[i % 4][KRK[q][i] ^ w2[i] ^ m_2];
      else t[i] = masking_Sbox[i % 4][KRK[q][i] ^ w2[i] ^ m_1];
   }
   DL(t, w3);
   for (i = 0; i < 16; i++) {
      if (i == 0 || i == 1 || i == 6 || i == 7 || i == 8 || i == 9 || i == 14 || i == 15)   w3[i] ^= w1[i] ^ m_1;
      else w3[i] ^= w1[i] ^ m_2;
   }

   for (i = 0; i < 16 * (R + 1); i++) e[i] = 0;

   RotXOR(w0, 0, e); RotXOR(w1, 19, e);
   RotXOR(w1, 0, e + 16); RotXOR(w2, 19, e + 16);
   RotXOR(w2, 0, e + 32); RotXOR(w3, 19, e + 32);
   RotXOR(w3, 0, e + 48); RotXOR(w0, 19, e + 48);
   RotXOR(w0, 0, e + 64); RotXOR(w1, 31, e + 64);
   RotXOR(w1, 0, e + 80); RotXOR(w2, 31, e + 80);
   RotXOR(w2, 0, e + 96); RotXOR(w3, 31, e + 96);
   RotXOR(w3, 0, e + 112); RotXOR(w0, 31, e + 112);
   RotXOR(w0, 0, e + 128); RotXOR(w1, 67, e + 128);
   RotXOR(w1, 0, e + 144); RotXOR(w2, 67, e + 144);
   RotXOR(w2, 0, e + 160); RotXOR(w3, 67, e + 160);
   RotXOR(w3, 0, e + 176); RotXOR(w0, 67, e + 176);
   RotXOR(w0, 0, e + 192); RotXOR(w1, 97, e + 192);
   if (R > 12) {
      RotXOR(w1, 0, e + 208); RotXOR(w2, 97, e + 208);
      RotXOR(w2, 0, e + 224); RotXOR(w3, 97, e + 224);
   }
   if (R > 14) {
      RotXOR(w3, 0, e + 240); RotXOR(w0, 97, e + 240);
      RotXOR(w0, 0, e + 256); RotXOR(w1, 109, e + 256);
   }
   Byte s1 = (masking_xor >> 3) | (masking_xor << (8 - 3));
   Byte s2 = (masking_xor >> 7) | (masking_xor << (8 - 7));
   Byte s3 = (masking_xor << 5) | (masking_xor >> (8 - 5));
   Byte s4 = (masking_xor << 7) | (masking_xor >> (8 - 7));

   for (i = 0; i < 64; i++) e[i] ^= s1;
   for (i = 64; i < 128; i++) e[i] ^= s2;
   for (i = 128; i < 192; i++)e[i] ^= s3;
   for (i = 192; i < 256; i++)e[i] ^= s4;

   return R;

}

int DecKeySetup( Byte *w0, Byte *d, int keyBits,Byte m_1,Byte m_2,Byte masking_xor) {
   int  i, j, R;
   Byte t[16];
  
   R=EncKeySetup(w0, d, keyBits,m_1,m_2,masking_xor);   
   for (j = 0; j < 16; j++){
      t[j] = d[j];
      d[j] = d[16*R + j];
      d[16*R + j] = t[j];
   }
   for (i = 1; i <= R/2; i++){
      DL (d + i*16, t);
      DL (d + (R-i)*16, d + i*16);
      for (j = 0; j < 16; j++) d[(R-i)*16 + j] = t[j];
   }
   return R;
}

void Crypt( Byte *p, int R,  Byte *e, Byte *c,Byte m_1,Byte m_2,Byte masking_xor)
{
   int i, j;
   Byte t[16];
  
   for (j = 0; j < 16; j++) c[j] = p[j];   
   for (j = 0; j < 16; j+=4) {
      c[j] ^= m_2;
      c[j+1] ^= m_2;
      c[j+2] ^= m_1;
      c[j+3] ^= m_1;

   }

   for (i = 0; i < R/2; i++)
   {
      for (j = 0; j < 16; j++)
        t[j] = masking_Sbox[j % 4][e[j] ^ c[j]];
      DL(t, c);
      for (int k = 0; k < 16; k++) {
         if (k % 8 > 3 && k % 8 < 8) c[k] ^= masking_xor;
      }
      e += 16;
      for (j = 0; j < 16; j++) t[j] = masking_Sbox[(2 + j) % 4][e[j] ^ c[j]];
      DL(t, c); 
      for (int k = 0; k < 16; k++) {
         if (k % 8 > 3 && k % 8 < 8) c[k] ^= masking_xor;
      }
      e += 16;
   }
   for (int k = 0; k < 16; k++) {
      if (k % 8 > 3 && k % 8 < 8) c[k] ^= masking_xor;
   }   DL(c, t);
   for (j = 0; j < 16; j++) c[j] = e[j] ^ t[j];

   for (j = 0; j < 16; j += 4) {
      c[j] ^= m_2;
      c[j + 1] ^= m_2;
      c[j + 2] ^= m_1;
      c[j + 3] ^= m_1;
   }
}

void ARIA_test(Byte m_1, Byte m_2, Byte masking_xor) {
    Byte rk[16 * 17], c[16];
    Byte mk[32] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
    };
    Byte p[16] = {
        0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa,
        0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb
    };
    Byte cryptResult[] = {
        0xc6, 0xec, 0xd0, 0x8e, 0x22, 0xc3, 0x0a, 0xbd,
        0xb2, 0x15, 0xcf, 0x74, 0xe2, 0x07, 0x5e, 0x6e
    };

    printf("BEGIN testing basic encryption...\n");
    printf("key      : "); printBlock(mk);
    Crypt(p, EncKeySetup(mk, rk, 128, m_1, m_2, masking_xor), rk, c, m_1, m_2, masking_xor);
    printf("plaintext: "); printBlock(p);
    printf("result is: "); printBlock(c);
    printf("should be: "); printBlock((Byte*)cryptResult);

    int flag = 0;
    for (int i = 0; i < 16; i++)
        if (c[i] != cryptResult[i])
            flag = 1;

    if (flag)
        printf("The result is incorrect!\n");
    else
        printf("Okay.  The result is correct.\n");
    printf("END testing basic encryption.\n\n");
}

int main() {
    Byte m_1 = 1, m_2 = 2, masking_xor = m_1 ^ m_2;
    generateMaskingSbox(S, m_1, m_2, masking_Sbox);
    ARIA_test(m_1, m_2, masking_xor);
    return 0;
}
